...
call the concrete single-bond null-representative helper with literal `{true}`
locality hypotheses.  It adds no public theorem; the `example` is a compile-time
oracle for THE-ERIKSSON-PROGRAMME-shaped code.
-/
